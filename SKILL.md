---
name: research-radar
description: Lightweight AI research radar assistant. 用于按固定频道搜索和筛选近期有价值的 LLM quantization、AI infra、serving/runtime、training systems、MoE、communication、kernel/operator、hardware affinity、model releases 和 framework releases 相关论文、项目、博客或 artifact。
---
...
- 默认频道画像来自 `config/channels/<channel>/profile.yml`。常规 radar 优先只读 profile，以节省 token。
- 重点实体来自 [config/watchlist.yml](config/watchlist.yml)。只有在模型发布、框架 release、候选命中重点实体，或用户询问具体模型/机构时读取。
...
- 消费关系：频道 profile 用于默认召回和过滤；watchlist 用于模型/框架发布监控；full topics / research map 用于按需扩展。
