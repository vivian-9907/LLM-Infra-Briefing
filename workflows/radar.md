...
从固定频道的研究范围内发现有价值的论文、项目、博客、benchmark 和 artifact，输出精选候选清单。不要为了数量填充低价值条目。
...
- `source_scope`：全部来源、只查 arXiv、只查 GitHub、只查 Hugging Face、只查 RSS，或 `config/sources.yml` 中定义的来源组。

默认不要要求用户提供临时关键词。主题、同义词、包含词和排除词都来自 `config/channels/<channel>/topics.yml`。
...
如果用户的领域限定无法映射到现有子类，先用频道 topics 的同义词和频道研究地图做近似归类；仍不确定时，在输出中明确说明本轮采用的解释口径。
...
- 同一子类下只有 1 个候选，但它同时具备强证据：清晰论文/项目、明确 benchmark 或 artifact、明显工程落地信号，并且和频道 research map 中的重要方向直接相关。
- 用户明确要求对某个子类做专项观察。
...
1. 读取 `config/channels.yml`，确定本轮 `channel`，再读取该频道的 topics 和 research map。
2. 读取 `config/sources.yml`、`config/radar-rubric.yml` 和频道 research map。
3. 在指定时间范围和来源范围内搜索。
4. 如果用户给出领域限定，先按“领域限定与软过滤”确定本轮核心子类和允许保留的相邻子类。
5. 先用频道 topics 和 `config/radar-rubric.yml` 的 `prefilter_gates` 做主题过滤；未通过过滤的条目直接丢弃或标记为 `ignore`。
6. 对领域限定场景，再用软过滤判断条目是核心候选、相邻候选还是应过滤条目；相邻候选必须能解释其关系。
7. 将通过过滤的结果整理成 Discover Card：
   - 标题
   - 链接
   - 来源
   - 发布或更新时间
   - 类型：论文、代码、博客、benchmark、数据集或报告
   - 子类：使用本频道 topics / research map 中的子类
...
15. 输出候选总览表，便于快速比较不同方向的时间、工作、类型、数据格式/压缩率、核心技术、意义和落地状态。
16. 只有当“本轮方向摘要”已经点名某个子类达到门槛时，才补充“专项洞察”。专项洞察要总结方向级信号、共同点、分歧和下一步，而不是重复单条候选。
17. 如果某个子类本轮信号明显，可以补充专项洞察；证据不足时必须写入“暂不展开的子类及原因”。
18. 如果用户已有历史表格或明确要求维护历史记录，可以追加“历史表格”；第一版默认只预留，不强制维护。
19. 如果没有高价值结果，直接说明；只有在有帮助时才列出少量 near-miss。
